...
class NextBusAgency {
  ...
  public function routeConfig($route_id) {
	...
	$path_order = array();
	$segments = array();
	$pathSegments = $xml->getELementsByTagName('path');
	...
	foreach ($pathSegments as $path) {
	  foreach ($path->getElementsByTagName('tag') as $tag) {
	    ...
	    $raw_start_tag = $parts[1];

	    if (count($parts) < 2 || strlen($parts[2]) <= 2) {
	      $numSegments--;
	      continue;
	    }

	    $repeats = 0;
	    while (array_key_exists($raw_start_tag . $repeats, $path_order))
	      $repeats++;

	    $start_tag = $raw_start_tag . $repeats;

	    if ($parts[2] == 'loop') {
	      $next_tag = $raw_start_tag . ($repeats + 1);
	    }
	    else {
	      $repeats = 0;
	      $next_tag = $parts[2];
	      while (array_search($next_tag . $repeats, $path_order)) {
		$repeats++;
	      }
	      $next_tag = $next_tag . $repeats;
	    }

	    $path_order[$start_tag] = $next_tag;
	    $segment = array();
	    ...
	    $segments[$start_tag] = $segment;
	  }
	} // path
	$startTag = reset($path_order);
	for ($i = 0; $i < $numSegments; $i++) {
          if (!array_key_exists($startTag, $segments)) {
            $startTag = next($path_order);
            continue;
          }
	  $segment = $segments[$startTag];
	  $startTag = $path_order[$startTag];
