...
class ShuttleTrip extends ShuttleObject {
  ...
  public function getFrequency($time=NULL) {
    if ($time === NULL) $time = time();
    if (!$this->isRunning($time)) {
      $time = $this->nextTripStart($time);
    }
  ...
  public function nextTripStart($time) {
    ...
    $nextStart = $secsPastMidnight + $midnight;
    if ($nextStart < $time) {
      $nextStart = $secsPastMidnight + $this->service->nextStartDate($time + 86400);
    }
    return $nextStart;
  }
