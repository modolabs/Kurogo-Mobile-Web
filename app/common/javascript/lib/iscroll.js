 ...
 * Changes added by Mark Edwards - Last updated: 2011.01.22
 * 
 */
...
function iScroll (el, options) {
	...
	that.element.style.webkitTransitionTimingFunction = 'cubic-bezier(0,0.15,0.4,1)';
...
iScroll.prototype = {
	...
	refresh: function () {
		...
		if (that.options.hScrollbar && that.scrollX) {
			that.scrollBarX = that.scrollBarX || new scrollbar('horizontal', that.wrapper, that.options.fadeScrollbar, that.options.stickyScrollbar, that.options.shrinkScrollbar, that.options.scrollbarColor, that.options.scrollbarWidth);
			that.scrollBarX.init(that.scrollWidth, that.scrollerWidth, that.options.scrollbarWidth);
		...
		if (that.options.vScrollbar && that.scrollY && that.scrollerHeight > that.scrollHeight) {
			that.scrollBarY = that.scrollBarY || new scrollbar('vertical', that.wrapper, that.options.fadeScrollbar, that.options.stickyScrollbar, that.options.shrinkScrollbar, that.options.scrollbarColor, that.options.scrollbarWidth);
			that.scrollBarY.init(that.scrollHeight, that.scrollerHeight, that.options.scrollbarWidth);
	...
	touchStart: function(e) {
		...
		if (nodeName == 'SELECT' || nodeName == 'INPUT' || nodeName == 'BUTTON') {
		  return;
		}

		e.preventDefault();
		if (!that.options.allowPropagation) { e.stopPropagation(); }
	...
	touchMove: function(e) {
		...
		// Prevent false clicks when scrolling, and stop propogation if desired
		e.preventDefault();
		if (!that.options.allowPropagation) { e.stopPropagation(); }
		...
		// Don't trigger click on a swipe greater than clickThreshold
		if ((Math.abs(pageX - that.fixedStartX) + Math.abs(pageY - that.fixedStartY)) > that.options.clickThreshold) { that.swiped = true; }
		...
		if (that.distX + that.distY > that.options.scrollThreshold) {
			
			that.onScrollStart();						// Execute custom code on scroll start
...
//			that.dist+= Math.abs(leftDelta) + Math.abs(topDelta);
	...
	touchEnd: function(e) {
		...
		if (!that.moved && !that.swiped && that.options.allowClick) {
			that.resetPosition();
	...
	resetPosition: function () {
		...
		if (resetX != that.x || resetY != that.y) {
			...
			// Hide the scrollbars
			if (that.scrollBarX && !that.options.stickyScrollbar) {
				that.scrollBarX.hide();
			}
			if (that.scrollBarY && !that.options.stickyScrollbar) {
	...
	scrollTo: function (destX, destY, runtime) {
		...
		var originalTiming = that.element.style.webkitTransitionTimingFunction;
...
function scrollbar (dir, wrapper, fade, sticky, shrink, color, width) {
	var that = this,
		doc = document;
	...
	that.bar.style.cssText = 'position:absolute;top:0;left:0;-webkit-transition-timing-function:cubic-bezier(0,0,0.25,1);pointer-events:none;-webkit-transition-duration:0;-webkit-transition-delay:0;-webkit-transition-property:-webkit-transform;z-index:10;background:' + color + ';' +
		...
		(dir == 'horizontal' ? '-webkit-border-radius:3px 2px;min-width:6px;min-height:' + width + 'px' : '-webkit-border-radius:2px 3px;min-width:' + width + 'px;min-height:6px');

	// Create scrollbar wrapper
	that.wrapper = doc.createElement('div');
	that.wrapper.style.cssText = '-webkit-mask:-webkit-canvas(scrollbar' + that.uid + that.dir + ');position:absolute;z-index:10;pointer-events:none;overflow:hidden;' + (sticky ? 'opacity:1;' : 'opacity:0;') + '-webkit-transition-duration:' + (fade ? '300ms' : '0') + ';-webkit-transition-delay:0;-webkit-transition-property:opacity;' +
		(that.dir == 'horizontal' ? 'bottom:2px;left:2px;right:7px;height:' + width + 'px' : 'top:2px;right:1px;bottom:7px;width:' + width + 'px;');
...
scrollbar.prototype = {
	init: function (scroll, size, width) {
		var that = this,
			doc = document,
			pi = Math.PI,
			radius = width / 2,
		...
		if (that.dir == 'horizontal') {
			if (that.maxSize != that.wrapper.offsetWidth) {
				...
				ctx = doc.getCSSCanvasContext("2d", "scrollbar" + that.uid + that.dir, that.maxSize, width);
				ctx.fillStyle = "rgb(0,0,0)";
				ctx.beginPath();
				ctx.arc(radius, radius, radius, pi/2, -pi/2, false);
				ctx.lineTo(that.maxSize-radius, 0);
				ctx.arc(that.maxSize-radius, radius, radius, -pi/2, pi/2, false);
				ctx.closePath();
				ctx.fill();
			}
		} else {
			if (that.maxSize != that.wrapper.offsetHeight) {
				...
				ctx = doc.getCSSCanvasContext("2d", "scrollbar" + that.uid + that.dir, width, that.maxSize);
				ctx.fillStyle = "rgb(0,0,0)";
				ctx.beginPath();
				ctx.arc(radius, radius, radius, pi, 0, false);
				ctx.lineTo(width, that.maxSize-radius);
				ctx.arc(radius, that.maxSize-radius, radius, 0, pi, false);
				ctx.closePath();
				ctx.fill();
			}
		}
