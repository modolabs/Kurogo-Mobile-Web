...
function showCalloutForPlacemark(placemark) {
    placemark.infoWindow.open(map, placemark);
    if (calloutListener !== null) {
        google.maps.event.removeListener(calloutListener);
    }
    calloutListener = google.maps.event.addDomListener(map, 'click', function() {
        placemark.infoWindow.close();
    });
}
